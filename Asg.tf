...
resource "aws_launch_template" "webserver_launch_template" {
  ...
  monitoring {
    enabled = true     # for detailed monitoring
  }
...
resource "aws_autoscaling_group" "auto_scaling_group" {
  vpc_zone_identifier = [aws_subnet.private_app_subnet_az1.id, aws_subnet.private_app_subnet_az2.id]                           # the subnet where you want tolaunch the ec2
  desired_capacity    = 2
  max_size            = 4
  min_size            = 1
  name                = "kelly-asg"
  ...
  launch_template {
    ...
    version = "$Latest"                        # launches the latest version of the launch template
  }

  tag {
    key                 = "name"
    value               = "asg-webserver"
    propagate_at_launch = true          # tags the ec2 with name above at launch
  }
