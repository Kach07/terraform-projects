...
resource "aws_eip" "eip_for_nat_gateway_az1" {
 vpc     = true                                                                 # it will be assocuiated with a vpc     
...
resource "aws_nat_gateway" "nat_gateway_az1" {
  allocation_id = aws_eip.eip_for_nat_gateway_az1.id        #we allocate the eip to the nat-gateway
  ...
  depends_on = [aws_internet_gateway.internet_gateway]     # this means nat-gateway must be delete B4 the internet gateway is deleted
}
